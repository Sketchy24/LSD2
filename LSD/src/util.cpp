...
char app[20];
strcpy(s, var->label);
for(cur=var->up; cur->up!=NULL; cur=cur->up); //Find the root
ceil=cur;
if(var->up==ceil) //The variable is in Root
  strcat(s, "_R");
else
while(var->up!=ceil)
{for(cur=var->up; cur->up!=ceil; cur=cur->up);
 sprintf(app, "_%d", cur->counter);
 strcat(s, app);

 for(cur1=var->up; cur1->up!=ceil; cur1=cur1->up);
 ceil=cur1;
}

/*
for(cur=var->up; cur!=NULL; cur=cur->up)
 {sprintf(app, "_%d", cur->counter);
  strcat(s, app);
 }
*/
...
for(cv=r->v; cv!=NULL; cv=cv->next)
  {
  if(cv->save==1)
   {sprintf(tag, "%s", cv->lab_tit+strlen(cv->label)+1);
   if(header==1)
     fprintf(f, "%s %s (%d %d)\t",cv->label, tag, cv->start, cv->end);
   else
     fprintf(f, "%s %s (-1 -1)\t",cv->label, tag);
   }
 }
