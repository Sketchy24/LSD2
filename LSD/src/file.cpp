...
for(cv=v; cv!=NULL; cv=cv->next)
{
 ...
 if(cv->param==0)
	fprintf(f, "Var: %s %d %c %c %c %c", cv->label, cv->num_lag, cv->save==1?'s':'n', ch, cv->debug,cv->plot==1?'p':'n');
 if(cv->param==1)
	fprintf(f, "Param: %s %d %c %c %c %c", cv->label, cv->num_lag, cv->save==1?'s':'n', ch, cv->debug,cv->plot==1?'p':'n');
 if(cv->param==2)
	fprintf(f, "Func: %s %d %c %c %c %c", cv->label, cv->num_lag, cv->save==1?'s':'n', ch, cv->debug,cv->plot==1?'p':'n');
