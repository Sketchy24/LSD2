...
if(*choice>0)
  cmd(inter, "bind .s.i.f <Down> {set bidi 4; focus -force .s.i.c}");
cmd(inter, "bind .s.i.f <Up> {set bidi 0; focus -force .s.i.m}");

cmd(inter, "radiobutton .s.i.c -text \"Create Series from selected series\" -variable bidi -value 4");
cmd(inter, "bind .s.i.c <Up> {set bidi 1; focus -force .s.i.f}");

if(*choice>0)
 cmd(inter, "pack .s.i.m .s.i.f .s.i.c -anchor w");
else
...
/**********************
Generate the tag to associate to the series fetched from the current model
**********************/
void print_tag(object *r, char *ch)
{ 
/*****
THIS WORKS, BUT IT IT SLOW
object *cur;
int i, j;
...
print_tag(r->up, ch);
if(r->up->up==NULL)
 sprintf(ch,"%d", r->t_counter);
else
 {
 sprintf(msg,"_%d", r->t_counter); 
 strcat(ch, msg);
 }
   ...
   if(cv->save==0 && !strcmp(cv->label,lab))
    {
     ...
     if(cur->up==NULL)
       ...
       print_tag(cur, lab_tit);
     sprintf(msg, ".f.vars.lb.v insert end \"%s %s (%d - %d) # %d\"", cv->label, lab_tit, 0, 0, num_var + (*num_v));
     cmd(inter, msg);
     sprintf(msg, ".f.vars.ch.v insert end \"%s %s (%d - %d) # %d\"", cv->label, lab_tit, 0, 0, num_var + (*num_v));
     cmd(inter, msg);
...
//     sprintf(msg, ".f.vars.lb.v insert end \"%s %s (%d - %d) # %d\"", cv->label, cv->lab_tit+strlen(cv->label)+1, cv->start, cv->end, *num_v); //this uses the tags embdedded in lab_tit
     sprintf(msg, ".f.vars.lb.v insert end \"%s %s (%d - %d) # %d\"", cv->label, mytag, cv->start, cv->end, *num_v);
     if(cv->end>*num_c)
       *num_c=cv->end;
     cmd(inter, msg);
     *num_v+=1;
...
if(r->up==NULL)
 for(cv=cemetery; cv!=NULL; cv=cv->next)
  {  sprintf(msg, ".f.vars.lb.v insert end \"%s %s (%d - %d) # %d\"", cv->label, cv->lab_tit+strlen(cv->label)+1, cv->start, cv->end, *num_v);
     if(cv->end>*num_c)
       *num_c=cv->end;
     cmd(inter, msg);
     *num_v+=1;
...
for(cv=r->v; cv!=NULL; cv=cv->next)
 if(cv->save)
  {
   strcpy(vs[*num_v].label,cv->label);
   strcpy(vs[*num_v].tag,cv->lab_tit+strlen(cv->label)+1);
...
cmd(inter, "pack .s.o.l .s.o.c .s.o.b .s.o.g .s.o.p -expand yes -fill x -anchor w");
