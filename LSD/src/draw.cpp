...
cmd(inter, ch);
*/
sprintf(ch, "$c.c bind %s <Enter> {if {[winfo exists .list]==1} {destroy .list} {};toplevel .list ;label .list.l -text \"$list_%s\" -justify left; pack .list.l;  align .list . }", str2, str2);
cmd(inter, ch);
sprintf(ch, "$c.c bind %s <Leave> {destroy .list}", str2);
cmd(inter, ch);

sprintf(ch, "$c.c bind %s <Double-Button-1> {set res_g %s; set choice_g 24}", str2, str2);
cmd(inter, ch);
sprintf(ch, "$c.c bind %s <Button-3> {wm withdraw $c; set res_g %s; set choice_g 25; }", str2, str2);
cmd(inter, ch);
sprintf(ch, "$c.c bind %s <Button-2> {wm withdraw $c; set res_g %s; set choice_g 25; }", str2, str2);
cmd(inter, ch);
