...
cmd(inter, "$w add command -label \"Run Model\" -state disabled -underline 0 -command {set choice 2} -accelerator Control+r");
cmd(inter, "$w add command -label \"gdb Debug\" -state disabled -underline 0 -command {set choice 13}");
cmd(inter, "$w add separator");
cmd(inter, "$w add command -label \"Show Equation\" -state disabled -underline 5 -command {set choice 8} -accelerator Control+e");
...
cmd(inter, "$w add command -label \"About LMM + Lsd\" -command {if { [winfo exists .about]==1} {destroy .about } {}; toplevel .about; label .about.l -text \"Version 5.9 \n\nOctober 2009\n\n\"; button .about.ok -text \"Ok\" -command {destroy .about}; pack .about.l .about.ok; wm title .about \"\"}"); 
...
if( tosave==1 && (choice==2 || choice==15 || choice==1 ||choice==14 ||choice==6 ||choice==8 ||choice==3 ||choice==7||choice==33||choice==5||choice==39||choice==41))
  {

//  cmd(inter, "set answer [tk_dialog .dia \"Save?\" \"Save the file $filename?\" \"\" 0 yes no cancel]");
  cmd(inter, "set answer [tk_messageBox -type yesnocancel -title \"Save file?\" -message \"Save  file '$filename'?\"]");
...
cmd(inter, "toplevel .mm; text .mm.t -yscrollcommand \".mm.yscroll set\" -wrap word; scrollbar .mm.yscroll -command \".mm.t yview\"");

cmd(inter, "pack .mm.yscroll -side right -fill y; pack .mm.t -expand yes -fill both; wm geom .mm -0+0; wm title .mm \"Compilation Results\"");
